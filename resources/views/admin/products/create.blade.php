...
<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <h1 class="text-4xl font-bold text-gray-900 mb-8">Create New Product</h1>

    <form action="{{ route('admin.products.store') }}" method="POST" class="bg-white rounded-lg shadow-md p-8">
        @csrf
        ...
        <div class="mb-6">
            ...
            <input type="number" id="stock" name="stock" value="{{ old('stock') }}" min="0" class="w-full px-4 py-2 border border-gray-300 rounded-lg @error('stock') border-red-500 @enderror" required>
            @error('stock')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-8">
            ...
            <input type="text" id="image_path" name="image_path" value="{{ old('image_path') }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg @error('image_path') border-red-500 @enderror" placeholder="products/image.jpg">
            @error('image_path')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
